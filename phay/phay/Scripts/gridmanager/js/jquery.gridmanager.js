          ...
          var canvas=gm.$el.find("#" + gm.options.canvasId);
           ...
           gm.$el.on("click", ".gm-preview", function(){
            ...
            // Switch editing mode
                   ...
                   if (gm.mode === "visual") {
                       gm.deinitCanvas();
                       canvas.html($('<textarea/>').attr("id", "LayoutCode").attr("name", "LayoutCode").attr("cols", 130).attr("rows", 25).val(canvas.html()));
                       gm.mode = "html";
            ...
            // Remove a class from a row via rowsettings
             ...
             /* Col settings */
              ...
              // Cap nhat ten Widget
               ...
               .on("click", "a.gm-colWidget", function () {
                   var col = $(this).closest(gm.options.colSelector);
                   var drawer = col.find(".gm-colSettingsDrawer");
                   if (drawer.length > 0) {
                       drawer.remove();
                   } else {
                       col.prepend(gm.generateColWidget(col));
                   }

                   // Change Col ID via colsettings
               })
               .on("blur", "input.gm-colWidgetID", function () {

                   var parentwidget = $(this).closest('.gm-editing');
                   var tagWidget = parentwidget.find(".post_widget");
                   var nameVT = "§§§_" + $(this).val() + "_§§§";
                   tagWidget.html(nameVT);

                   // $(this).parents('table').find('tr').removeClass('trChecked');
               ...
               .on("click", "a.gm-resetgrid, a.gm-remove, a.gm-removeRow, a.gm-save, button.gm-preview, a.gm-viewsource, a.gm-addColumn, a.gm-colDecrease, a.gm-colIncrease", function (e) {                
                ...
                gm.log("Clicked: " + $.grep((this).className.split(" "), function (v) {
                 return v.indexOf('gm-') === 0;
                    ...
                    //.on("click", "a.gm-removeRow, a.gm-removeCol, a.gm-resetgrid, a.gm-colIncrease, a.gm-colDecrease, a.gm-addRow, a.gm-addColumn, .gm-togglecolClass, input.gm-colSettingsID, a.gm-colSettings, .gm-toggleRowClass, input.gm-rowSettingsID, a.gm-rowSettings, a.gm-save, .gm-edit-mode, .gm-layout-mode a, .gm-preview", function () {
...
/*------------------------------------------ ROWS ---------------------------------------*/
        ...
        gm.generateColWidget = function (col) {
            // Col class toggle buttons
            var classBtns = [];
            $.each(gm.options.colCustomClasses, function (i, val) {
                var btn = $("<button/>")
            ...
            var html = $("<div/>")
                ...
                //  .html(classBtns.join("")))
                .append($("<div />").addClass("pull-right").html(
                    $("<label />").html("Tên vị trí ").append(
                        $("<input>")
                            .addClass("gm-colWidgetID")
                            .attr({ type: 'text', placeholder: 'Nhập tên vị trí ', value: col.attr("id") })
                    )
                ));
        ...
        /**
         ...
         gm.createCol =  function(size){
            ...
            .addClass(gm.options.colAdditionalClass)
                 .html(gm.toolFactory(gm.options.colButtonsPrepend) + "<div class=\"post_widget\">§§§_ViTriChen_§§§</div>")
...
/*------------------------------------------ Useful functions ---------------------------------------*/
    ...
    */
        ...
        // URL to save to
        remoteURL: "/CauHinh/CreateLayout",
       ...
       // Generic desktop size layout class
        ...
        colButtonsPrepend: [
            ...
            //},
            {
                title: "Tên widget",
                element: "a",
                btnClass: "pull-right gm-colWidget",
