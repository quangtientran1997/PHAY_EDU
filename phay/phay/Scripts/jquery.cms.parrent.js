...
function ajaxFormBeforeForGetDataForSubmit() { } ! function (e) {
    jQuery.fn.extend({
        ...
        gridDeleteOne: function (target, url, validateUrl, isReload = false) {
            toastr.clear();
            toastr.remove();
            ...
            toastr.warning("<div style='text-align:right;'><button type='button' class='btn grey-cascade btn-sm' style='margin-right:15px;'>Không</button><button type='button' id='confirmDeleteYes' class='btn blue btn-sm'>Đồng ý</button><div><div class='clearfix'></div>", 'Bạn có chắc chắn muốn xóa không?',
                {
                    closeButton: false,
                    allowHtml: true,
                    timeOut: 0,
                    ...
                    onShown: function (toast) {
                        $("#confirmDeleteYes").click(function () {
                            ...
                            $.ajaxPostModels(url, null, function (e) {
                                var successMsg = "Xóa thành công"; //$(target).getAttr("data-success-msg");
        ...
        gridDeleteList: function (target, url) {
            toastr.clear();
            toastr.remove();
            var arr = [];
        ...
        gridChangeBitList: function (target, url, title) {
            ...
            if (arr.length > 0) {
                ...
                toastr.warning("<div style='text-align:right;'><button type='button' class='btn grey-cascade btn-sm' style='margin-right:15px;'>Không</button><button type='button' id='confirmDeleteYes' class='btn blue btn-sm'>Đồng ý</button><div><div class='clearfix'></div>", 'Bạn có chắc chắn muốn thay đổi dữ liệu?',
                    {
                        closeButton: false,
                        allowHtml: true,
                        timeOut: 0,
        ...
        gridSave: function () {
            ...
            if ($(this).gridIsDirty()) {
                ...
                $.warning('Không có dữ liệu thay đổi');
            }
        },
...
function warning_parrent(t) {
    $.warning(t);
}
function info_parrent(t) {
    $.info(t);
}
function success_parrent(t) {
    $.success(t);
}
