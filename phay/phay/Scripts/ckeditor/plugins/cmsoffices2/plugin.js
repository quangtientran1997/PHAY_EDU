...
CKEDITOR.plugins.add('cmsoffices2', {
    icons: 'cmsoffices',
    init: (editor) => {
        _editor = editor;
        editor.ui.addButton('cmsoffices', {
            ...
            toolbar: 'insert',
            icon: editor.plugins.cmsoffices2.path + 'images/cmsoffices.png'
        });
...
function showPopupOfficeBrowser() {
    $(BASE_POPUP).val("files_array_arrfile");
        ...
        if (files !== null/* && files.length > 0*/) {
            ...
            $content.html(`
                 <table class="table table-bordered">
                    <thead>
                      <tr>
                        <th style="width:80px;text-align: center;">Xem trực tuyến</th>
                        <th style="width: 250px;text-align: center;">Tên tập tin</th>
                        <th style="width: 250px;text-align: center;">Tập tin</th>
                      </tr>
                    </thead>
                    <tbody>
            ...
            fileSelectedArray = files.file.map((item, index) => {
            ...
            files.file.forEach((item, index) => {
                ...
                $content.find('.table > tbody').append(`
                      <tr>
                        ...
                        <td style="vertical-align: middle;"><input type="text" id="file-name-${item.idFile}" class="k-textbox fill-input form-control" value="${item.name}" /></td>
                        <td style="vertical-align: middle;"><span style="white-space: nowrap;width: 100% !important;overflow:hidden;text-overflow: ellipsis;display: inline-block;">${item.file}</spa></td>
                      </tr>
            `);
            });
...
function getSource(file_name, extension) {
    ...
    try {
        _urlUpload = localStorage.getItem("ul");
    }
    catch{
    }
    var embed = '';
    if (officeExtensions.indexOf(extension) > -1) {
        if (_urlUpload != null && _urlUpload != '' && _urlUpload != undefined) {
            embed = `https://docs.google.com/viewer?url=${_urlUpload}/${file_name}`
        }
        else {
...
function insertFiles() {
    fileSelectedArray.forEach((item, index) => {
        ...
        if (item.isDisplay) {
            ...
            link.setAttributes({ 'target': '_blank' });
